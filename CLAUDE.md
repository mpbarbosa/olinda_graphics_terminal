...
- `src/manual.ts` (`createManViewer`) owns the panel: fetches + caches per command, guards against out-of-order responses with a request sequence number, and opens/collapses the panel (calling `onLayoutChange` so the terminal can refit).
- `src/main.ts` detects the command: it reads the **cursor's buffer row** (`buffer.active.getLine(baseY + cursorY)`), strips the prompt marker (`[❯➜»%$#>]`), and takes the first token. The lookup is debounced (300ms) and gated on the token being in the shared PATH command set (`commandSet`), so partial typing and non-commands don't fire fetches.
